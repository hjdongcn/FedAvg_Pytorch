...
class GetDataSet(object):
    def __init__(self, dataSetName, isIID):  # 构造函数 
    ...
    def mnistDataSetConstruct(self, isIID):
        ...
        print(train_images.shape)  # mnist集 6000*28*28*1 1通道
        print(train_labels.shape)  # 6000*10 转化成了one-hot
        ...
        train_images = train_images.reshape(train_images.shape[0], train_images.shape[1] * train_images.shape[2])  # reshape(a, b) a行b列的矩阵
        test_images = test_images.reshape(test_images.shape[0], test_images.shape[1] * test_images.shape[2])

        train_images = train_images.astype(np.float32)  # 重新编码
        train_images = np.multiply(train_images, 1.0 / 255.0)  # 归一化
        test_images = test_images.astype(np.float32)
        test_images = np.multiply(test_images, 1.0 / 255.0)

        if isIID:
            order = np.arange(self.train_data_size)  # 一个有终点和起点的固定步长的排列
...
# 经过GetDataSet处理过的数据集为一个实例，具有train_data, test_data, train_label, test_label， 各自的size，以及名字这几个属性。